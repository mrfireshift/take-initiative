...
const turnNotice = read("../src/turn-notice.ts");
...
test("le attivazioni usano un popup dedicato e non la Console HP", () => {
  ...
  assert.match(popupController, /directPointSelection: true/);
  assert.match(popupController, /snapToItemCenter: true/);
  assert.match(aoeTargetTool, /nearestGridFootprintCenter/);
  assert.match(aoeTargetTool, /function isDirectPointSelection/);
  assert.match(aoeTargetTool, /if \(isDirectPointSelection\(state\)\) \{\s*state\.ready = true/);
  assert.match(aoeTargetTool, /autoConfirmPoint/);
  assert.match(turnNotice, /directPointSelection: true/);
  assert.doesNotMatch(turnNotice, /hint\.textContent = "Scegli la destinazione del ritorno sulla mappa\."/);
  assert.match(turnNotice, /status\.hidden = true/);
  assert.doesNotMatch(popupHtml, /blinkFallback|blinkConfirmPlacement|blinkCancelPlacement/);
