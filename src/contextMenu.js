  ...
  const ICON_PARAGON = ICON_MARK;
  const ICON_EPIC = ICON_MARK;
  ...
  function hasEpicActive() {
  // attivo se il nodo esiste
  return { key: ["metadata", META_KEY, "epic"], operator: "!=", value: undefined };
  }
  ...
  async function toggleLegendaryDefault(itemIds) {
    ...
    if (!allHave) {
      ...
      if (blocked) {
       ...
       const blockedEpic = items.some(it => {
        const e = it.metadata?.[META_KEY]?.epic;
        return !!e;
      });
      if (blockedEpic) {
        console.warn("[legendary] impossibile attivare: token con Epic Boss attivo");
        return;
      }
  ...
  // Toggle Epic Boss sul token (fissa iniziativa a 20 quando ON)
// Vietato se il token ha Legendary o Paragon attivi
  ...
  await OBR.scene.items.updateItems(ids, (draft) => {
    for (const it of draft) {
      ...
      if (me.epic) {
        ...
        // accendi e imposta iniziativa a 20 come da regola
        me.epic = { enabled: 1 };
        me.initiative = 20;
      }
      it.metadata = { ...(it.metadata || {}), [META_KEY]: me };
