...
// cancella selezioni accidentali senza bloccare gli elementi interattivi
container.addEventListener("mousedown", (e) => {
  const t = e.target;
  const interactive = t.closest("input, textarea, [contenteditable='true'], button");
  if (!interactive) {
    e.preventDefault();
...
async function updateHP(itemId, nextHP, nextHPMax) {
  ...
  try {
    ...
    // ===== Ordina per iniziativa (desc)
    function sortByInitiative(entries) {
...
const isNext = e.__groupMembers
  ...
  const activeBadge = document.createElement("div");
    ...
    Object.assign(activeBadge.style, {
      position: "absolute",
      left: "24px",                 // spilla sul lato sinistro della card
      top: "50%",
      transform: "translateY(30%)", // spostala leggermente in basso
      width: "22px",
      height: "22px",
      borderRadius: "50%",
      display: "flex",
...
if (e.__groupCollapsed) {
  name.title = `${e.__groupBase} (${e.__groupCount})`;
  name.textContent = e.__groupBase + " ";
} else {
  name.textContent = e.name;
  name.title = e.name;
}

// Pill count: mostrala quando collassato OPPURE se è la prima card del gruppo
if ((e.__groupCollapsed || e.__groupFirst) && e.__groupCount > 1) {
...
if (e.__groupCollapsed) {
  ...
  Object.assign(chev.style, {
    position: "absolute",
    left: "4px",                // ← a sinistra del badge ⚔ (che sta a 24px)
    top: "50%",
    transform: "translateY(-50%)",
    width: "18px",
    height: "18px",
...
if (IS_GM && !e.__groupCollapsed) {
  ...
  hpBarWrap.style.left = "calc(19% + 60px)"; // spostata a destra della pill
  hpBarWrap.style.width = "100px";
  ...
  // === Editor inline con supporto +N / -N ===
  ...
  // 2) Marca questa pill come “in edit” (serve per lo switch pulito)
  __editingHPForId = e.id;
  pill.dataset.hpEditing = "1";

  // 3) Swallow del primo click di coda, così non chiude subito l’editor appena aperto
  const swallowFirstClick = (evt) => {
  ...
  // helper: interpreta +N/-N come delta, numeri come assoluti
  ...
  active?.scrollIntoView?.({ behavior: "smooth", block: "center", inline: "nearest" });
    ...
    // ===== Inizializzazione: crea ordine da iniziativa
    async function ensureState() {
    const state = await getSceneState();
    if (state) return;
    const sorted = sortByInitiative(await readEntries());
    await setSceneState({
...
async function reconcileStateWithItems() {
  ...
  // se non è cambiato nulla (ordine e current), esci
  if (state &&
  ...
  await setSceneState({ order: newOrder, current: newCurrent, round, seededGroups });
  return true;
}
...
OBR.scene.onMetadataChange(async (meta) => {
  ...
  try {
    ...
    console.warn("[initiative] follow-onMetadataChange failed:", e?.message || e);
  }
});
